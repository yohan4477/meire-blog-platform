...
export default function StockPriceChart({ 
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const periodParam = convertPeriod(timeRange);
      
      // 🚀 ULTRA: 3개 API를 완전 병렬로 호출 (대기시간 제거)
      const [priceResult, sentimentResult, postsResult] = await Promise.all([
        fetch(`/api/stock-price?ticker=${ticker}&period=${timeRange}`).then(r => r.json()),
        fetch(`/api/merry/stocks/${ticker}/sentiments?period=${periodParam}`).then(r => r.json()),
        fetch(`/api/merry/stocks/${ticker}/posts?limit=100&offset=0&period=${periodParam}`).then(r => r.json())
      ]);
      ...
      if (priceResult.success && priceResult.prices) {
        ...
        // 🚀 ULTRA: 메모리 최적화된 날짜별 그룹화 (Object.create 사용)
        ...
        if (postsResult.success && postsResult.data?.posts) {
          postsResult.data.posts.forEach((post: any) => {
            ...
            (postsByDate[postDate] = postsByDate[postDate] || []).push(post);
          });
        }
