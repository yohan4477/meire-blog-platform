...
async function analyzeSamsungAllPosts() {
  console.log('🚀 삼성전자 모든 포스트 감정 분석 시작...');
  ...
  console.log(`📊 발견된 삼성전자 관련 포스트: ${posts.length}개`);
  ...
  for (const post of posts) {
    ...
    if (analysis) {
      ...
      analyzedCount++;
      console.log(`✅ Post ${post.id}: ${analysis.sentiment} (신뢰도: ${(analysis.confidence * 100).toFixed(0)}%) - "${post.title.substring(0, 50)}..."`);
    }
  }
  ...
  console.log(`  - 평균 신뢰도: ${(summary.avg_confidence * 100).toFixed(1)}%`);
  console.log(`  - 새로 분석: ${analyzedCount}개`);
  console.log(`  - 기존 분석: ${skippedCount}개`);
  
  db.close();
  console.log('\n✅ 삼성전자 모든 포스트 감정 분석 완료!');
}
